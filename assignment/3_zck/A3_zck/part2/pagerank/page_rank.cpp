...
void pageRank(DistGraph &g, double* solution, double damping, double convergence) {
    ...
    std::vector<double> score_next(vertices_per_process, 0.0);    //update to zeros every loop    
    ...
    // while (test < 3)  {
        ...
        //     if (g.out_edges_num[j] == 0) nooutnode_pr += score_last[j];
        // }           
    ...
    #pragma omp parallel
        ...
        for (int dst_idx = startVertex; dst_idx < endVertex; ++dst_idx) {
            ...
            for (int idx = 0; idx < esize; idx++){
                ...
                score_next[dst_idx - startVertex] += score_last[src] / g.out_edges_num[src];
            }
        }
        double sum_part = (sum_nooutnode_pr  * damping + (1.0 - damping)) / double(totalVertices);
        ...
        if (worldRank == MASTER){
            #pragma omp parallel for reduction(+:diff) schedule(static, chunksize)
            for (int i = 0; i < totalVertices; ++i) {
