...
void DistGraph::setup() {
    ...
    // printf("pass get_vertex_edges_size\n");
    ...
    // printf("pass set_edges\n");
    // for (int v = start_vertex; v <= end_vertex; v++) {
    //     int idx = v - start_vertex;
    //     int out_size = out_edge_dst_size[idx];
    //     int in_size = in_edge_src_size[idx];
    //     for (int i = 0; i < out_size; i++) {
    //         printf("Process %d, %d->%d\n", world_rank, v, out_edge_dst[idx][i]);
    //     }
    //     for (int i = 0; i < in_size; i++) {
    //         printf("Process %d, %d<-%d\n", world_rank, v, in_edge_src[idx][i]);
    //     }
    // }
    // exit(1);
}
