...
// Assignment 4.  Adapted for use in benchmarking
// Changes indicated by macro BENCH
#ifndef BENCH
#define BENCH 0
#endif
...
// By writing to this at end of function, code won't get optimized
// away due to dead code elimination.
volatile int global_save;
#else
 ...
 */
#if BENCH
void high_compute_job(unsigned int seed) {
#else
 ...
 */
#if BENCH
void mini_compute_job(int number) {
#else
 ...
 */
#if BENCH
void cachefootprint_job(unsigned int seed) {
#else
void cachefootprint_job(const Request_msg& req, Response_msg& resp) {
#endif

#if BENCH
  // Adapt for GHC machines.  Cache indicated to be 20480 KB
  unsigned int L3_SIZE = 19 * 1000 * 1000; 
  //  std::cout << "Running cachefootprint_job with seed = " << seed << std::endl;
#else
  ...
  //   std::cout << " Created permutation of size " << n << std::endl;
  ...
  //  std::cout << " #operations = " << nIterations * n << std::endl;

  for (unsigned int i = 0; i < nIterations * n; i++) {
    p = (void **)(*p);
  }

  //  std::cout << " Completed operations " << std::endl;
...
void execute_work(int b) {
    ...
    switch(b) {
    case BENCH_WISDOM:
	high_compute_job(seed);
	break;
    case BENCH_PRIMES:
	count_primes_job(N);
	break;
    case BENCH_TELLME:
	mini_compute_job(seed);
	break;
    case BENCH_PROJECT:
	cachefootprint_job(seed);
	break;
    default:
        std::cout << "Unknown benchmark id " << b << std::endl;
    }
}
#else
/*
